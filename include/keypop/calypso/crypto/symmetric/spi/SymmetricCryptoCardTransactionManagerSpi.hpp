 ...
 *
 * <p>It defines the API needed by a terminal to perform the cryptographic
 * operations required by a Calypso card when using symmetric keys.
 *
 * <p>An instance of this interface can be obtained via the method {@link
 * SymmetricCryptoCardTransactionManagerFactorySpi#createCardTransactionManager(byte[],
 * boolean, List)}.
 *
 * @since 0.1.0
 */
...
public:
    /**
     * Initializes the crypto service context for operating a Secure Session
     * with a card and gets the terminal challenge.
     *
    ...
    /**
     * Stores the data needed to initialize the session MAC computation for a
     * Secure Session.
     *
     * @param openSecureSessionDataOut The data out from the card Open Secure
     * Session command.
     ...
     *
     * @param cardApdu A byte array containing either the input or output data
     * of a card command APDU.
     * @return null if the encryption is not activate, either the ciphered or
     * deciphered command data if the encryption is active.
     * @throw SymmetricCryptoException If an internal error occurred.
     * @throw SymmetricCryptoIOException If an IO error occurred when processing
     * a command.
     * @since 0.1.0
    ...
    /**
     * Finalizes the digest computation and returns the terminal part of the
     * session MAC.
     *
     ...
     * @since 0.1.0
     */
    virtual std::vector<uint8_t> finalizeTerminalSessionMac() = 0;

    /**
     * Generates the terminal part of the session MAC used for an early mutual
     * authentication.
     *
     * @return A byte array containing the terminal session MAC.
     * @throw SymmetricCryptoException If an internal error occurred.
     ...
     * @since 0.1.0
     */
    virtual std::vector<uint8_t> generateTerminalSessionMac() = 0;
    ...
    /**
     * Verifies the card part of the session MAC finalizing the mutual
     ...
     */
    virtual bool
    isCardSessionMacValid(const std::vector<uint8_t>& cardSessionMac)
        = 0;
     ...
     * @since 0.1.0
     */
    virtual void computeSvCommandSecurityData(
        const std::shared_ptr<SvCommandSecurityDataApi> data)
        = 0;
    ...
    /**
     * Computes a block of encrypted data to be sent to the card for an
     * enciphered PIN presentation.
     *
     * <p>Note: the {@code kif} and {@code kvc} parameters are ignored when PIN
     * verification is performed within a Secure Session.
     *
     ...
     * @return A byte array containing the encrypted data block to sent to the
     * card.
     * @throw SymmetricCryptoException If an internal error occurred.
     * @throw SymmetricCryptoIOException If an IO error occurred when processing
     * a command.
     * @since 0.1.0
     */
    virtual std::vector<uint8_t> cipherPinForPresentation(
    ...
    /**
     * Computes a block of encrypted data to be sent to the card for a PIN
     * modification.
     *
     * <p>Note: the {@code kif} and {@code kvc} parameters are ignored when PIN
     * modification is performed within a Secure Session.
     *
     * @param cardChallenge A byte array containing the card challenge.
     ...
     * @since 0.1.0
     */
    virtual std::vector<uint8_t> cipherPinForModification(
        const std::vector<uint8_t>& cardChallenge,
     ...
     * @since 0.1.0
     */
    virtual std::vector<uint8_t> generateCipheredCardKey(
        const std::vector<uint8_t>& cardChallenge,
    ...
    /**
     * Synchronizes data of the associated card transaction crypto extension if
     * needed.
     *
     * @throws SymmetricCryptoException If an internal error occurred.
     * @throws SymmetricCryptoIOException If an IO error occurred when
